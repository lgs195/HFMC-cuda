...
def launch_1photon(rng_states, U0, S0, s11_1, s12_1, s33_1, s43_1, nphotons, out_x, out_y, out_phi, out_theta,
                   I_eff, Q_eff, U_eff, V_eff, n_scat, mus, mua, albedo, P_0):
    """模拟一个光子的入射的随机过程"""
    ...
    if idx < nphotons:
        ...
        while photon_status == ALIVE:
            ...
            rnd3 = 0.0  # 判断粒径随机数
            while rnd3 == 0.0:
                rnd3 = xoroshiro128p_uniform_float32(rng_states, thread_id)
            ...
            # 如果满足布格尔定律则粒子透射
                ...
                # 否则发生散射
                    ...
                    while True:
                        ...
                        ithedeg = int(ithedeg + n_r * nangles)

                        I0 = s11_1[0 + n_r * nangles] * S10 + s12_1[0 + n_r * nangles] * (S11 * math.cos(2 * phi) + S12 * math.sin(2 * phi))
                        # print(I0)
...
########################################################################################################################
            ...
            # if z >= Zmax or absorb_flag == 1:
                # 从上表面之外的另外五个面射出 or 被吸收，则视为无效光子，不计入统计
                ...
                photon_status = DEAD
            elif z <= Zmin or Xmax <= x or x <= Xmin or Ymax <= y or y <= Ymin:
                # 从上表面射出，统计出射光子的各参数

                if scat_flag == 1 and U12 < 0:
                    # 统计参数
                    # print(x)
...
def main():
    ...
    #     m_imag = L_m_imag[j]
    # # 半径
    # L_radius = [0.1, 0.5, 1.0, 2.0, 4.0, 10.0]
    # for radius1 in L_radius:
    """parameters"""
    X0 = 100  # 网格划分(视野大小)
    Y0 = 100
    Z0 = 10
...
###############################################################
    ...
    s11_1, s12_1, s33_1, s43_1, qsca_1, qabs_1, mus_1, mua_1, albedo_1 = ScatParam.ScatParam(m, rho_1, radius1,
                                                                                             lambda1, nre_med,
                                                                                             nangles)
    s11_2, s12_2, s33_2, s43_2, qsca_2, qabs_2, mus_2, mua_2, albedo_2 = ScatParam.ScatParam(m, rho_2, radius2,
                                                                                             lambda1, nre_med,
                                                                                             nangles)
    s11_3, s12_3, s33_3, s43_3, qsca_3, qabs_3, mus_3, mua_3, albedo_3 = ScatParam.ScatParam(m, rho_3, radius3,
                                                                                             lambda1, nre_med,
                                                                                             nangles)
    # Mueller矩阵系数集合 第i个粒径的第j个角度的索引为i * nangles + j
    s11_0 = np.concatenate((s11_1, s11_2, s11_3), axis=0, dtype=float)
    s12_0 = np.concatenate((s12_1, s12_2, s12_3), axis=0, dtype=float)
    s33_0 = np.concatenate((s33_1, s33_2, s33_3), axis=0, dtype=float)
    s43_0 = np.concatenate((s43_1, s43_2, s43_3), axis=0, dtype=float)

    # print(len(s11_0))

    A1 = np.pi * radius1 * radius1
    A2 = np.pi * radius2 * radius2
    A3 = np.pi * radius3 * radius3

    P_tmp = A1 * rho_1 + A2 * rho_2 + A3 * rho_3  # 分母
    # 各粒径概率
    P_1 = A1 * rho_1 / P_tmp
    P_2 = A2 * rho_2 / P_tmp
    P_3 = A3 * rho_3 / P_tmp
    P_0 = np.array([P_1, P_2, P_3])
    # print(P_0)

    # 散射系数与吸收系数估计
    mus = mus_1 + mus_2 + mus_3  # 散射系数  m^-1
    mua = mua_1 + mua_2 + mua_3  # 吸收系数  m^-1

    albedo_0 = np.array([albedo_1, albedo_2, albedo_3])  # 反照率
    ...
    s11_1_device = cuda.to_device(s11_0)  # Mueller矩阵元素s11
    s12_1_device = cuda.to_device(s12_0)  # Mueller矩阵元素s12
    s33_1_device = cuda.to_device(s33_0)  # Mueller矩阵元素s33
    s43_1_device = cuda.to_device(s43_0)  # Mueller矩阵元素s43

    P_0_device = cuda.to_device(P_0)

    albedo_0_device = cuda.to_device(albedo_0)
    ...
    launch_1photon[blocks_per_grid, threads_per_block](rng_states, U0_device, S0_device, s11_1_device,
                                                       ...
                                                       U_device, V_device, n_scat, mus, mua, albedo_0_device,
                                                       P_0_device)
    cuda.synchronize()
    ...
    with open(f"../data/球壳MC/Al/0in/4um/多分散1/origin.csv", 'w', newline='') as old_csv:
